...
public final class GameMetrics implements IRenderable {
  ...
  public void trackRenderTime(final String name, final double renderTime, final RenderInfo... infos) {
  ...
  public void render(final Graphics2D g) {
      ...
      if (!this.renderMetrics.isEmpty()) {
          ...
          for (final RenderMetrics metric : this.renderMetrics) {
  ...
  private void updateMetrics() {
      ...
      if (currentMillis - this.lastNetworkTickTime >= 1000) {
          ...
          if (!this.bytesSent.isEmpty()) {
              final Optional<Long> sentOpt = this.bytesSent.parallelStream().reduce((n1, n2) -> n1 + n2);
          ...
          if (!this.bytesReceived.isEmpty()) {
              final Optional<Long> receivedOpt = this.bytesReceived.parallelStream().reduce((n1, n2) -> n1 + n2);
  ...
  private void drawTitle(final Graphics2D g, final String title) {
  ...
  private void drawMetric(final Graphics2D g, final String metric) {
  ...
  public class RenderMetrics {
      ...
      RenderMetrics(final String name, final double renderTime, final RenderInfo... infos) {
      ...
      public String toString() {
          ...
          if (!this.renderInfo.isEmpty()) {
              sb.append(" ");
              for (final RenderInfo info : this.getRenderInfos()) {
                  sb.append(info);
              }
          }
          return sb.toString();
  ...
  public static class RenderInfo {
      ...
      public RenderInfo(final String name, final Object value) {
      this.name = name;
      this.value = value;
    }
