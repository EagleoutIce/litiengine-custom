...
public final class GameRandom extends java.util.Random {
  ...
  public <T> T[] sample(final T[] array, final int amount, final boolean replacement) {
      ...
      final T[] sampled = (T[]) java.lang.reflect.Array.newInstance(array.getClass().getComponentType(), amount);

      if (!replacement) {
          final T[] copiedArray = ArrayUtilities.arrayCopy(array);
  ...
  public <T> Collection<T> sample(final Collection<T> collection, final int amount, final boolean replacement) {
      if (!replacement && collection.size() < amount) {
          throw new IllegalArgumentException(INVALID_AMOUNT_FOR_SAMPLING_WITHOUT_REPLACEMENT);
      }

      if (!replacement) {
          final List<T> copied = new ArrayList<>(collection);
          this.shuffle(copied);

          return copied.stream().limit(amount).collect(Collectors.toList());
      }

      final List<T> copied = new ArrayList<>(amount);
   ...
   */
  public int choose(final int... array) {
      if (array == null || array.length == 0) {
   ...
   *                                      When the specified array is null or empty.
   */
  public long choose(final long... array) {
      if (array == null || array.length == 0) {
          throw new IllegalArgumentException(ARRAY_MUST_NOT_BE_EMPTY);
      }
   ...
   *                                      When the specified array is null or empty.
   */
  public double choose(final double... array) {
      if (array == null || array.length == 0) {
          throw new IllegalArgumentException(ARRAY_MUST_NOT_BE_EMPTY);
      }
   ...
   *                                      When the specified array is null or empty.
   */
  public String choose(final String... array) {
      if (array == null || array.length == 0) {
          throw new IllegalArgumentException(ARRAY_MUST_NOT_BE_EMPTY);
      }
   ...
   */
  public <T> T choose(final Collection<T> coll) {
  ...
  public <T> void shuffle(final T[] array) {
      for (int i = 0; i < array.length; i++) {
          final int randomPosition = this.nextInt(array.length);
   ...
   *                  The array to be shuffled.
   */
  public void shuffle(final long[] array) {
      for (int i = 0; i < array.length; i++) {
          final int randomPosition = this.nextInt(array.length);
   ...
   *                  The array to be shuffled.
   */
  public void shuffle(final double[] array) {
      for (int i = 0; i < array.length; i++) {
          final int randomPosition = this.nextInt(array.length);
   ...
   */
  public <T> void shuffle(final List<T> coll) {
   ...
   * @see #nextSign()
   */
  public int shuffleSign(final int value) {
   ...
   * @see #nextSign()
   */
  public float shuffleSign(final float value) {
      return value * this.nextSign();
  }
   ...
   * @see #nextSign()
   */
  public long shuffleSign(final long value) {
      return value * this.nextSign();
  }
   ...
   * @see #nextSign()
   */
  public double shuffleSign(final double value) {
      return value * this.nextSign();
  }
  ...
  public <T extends Enum<?>> T next(final Class<T> clazz) {
      final int x = this.nextInt(clazz.getEnumConstants().length);
  ...
  public Point2D getLocation(final Ellipse2D circle) {
      final double radius = circle.getWidth() / 2.0;
  ...
  public Point2D getLocation(final Point2D start, final Point2D end) {
      final double rnd = this.nextDouble(start.distance(end));
   ...
   */
  public String nextAscii(final int length) {
   ...
   */
  public Color nextColor(final Color originalColor, final float colorVariance, final float alphaVariance) {
      if (originalColor == null) {
          return null;
      }
      final int red = MathUtilities.clamp((int) (originalColor.getRed() + (originalColor.getRed() * this.nextFloat(colorVariance) * this.nextSign())), 0, 255);
      final int green = MathUtilities.clamp((int) (originalColor.getGreen() + (originalColor.getGreen() * this.nextFloat(colorVariance) * this.nextSign())), 0, 255);
      final int blue = MathUtilities.clamp((int) (originalColor.getBlue() + (originalColor.getBlue() * this.nextFloat(colorVariance) * this.nextSign())), 0, 255);
      final int alpha = MathUtilities.clamp((int) (originalColor.getAlpha() + (originalColor.getAlpha() * this.nextFloat(alphaVariance) * this.nextSign())), 0, 255);
